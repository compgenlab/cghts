...
// An optional member that is absent is an answer; one that exists but cannot be
// read is a failure. openOptionalMember used to discard every error alike, so a
// member with bad permissions, an I/O error or a symlink loop was reported as
// one that had never been written -- and the store opened with a quietly
// missing half rather than saying what was wrong.
func TestUnreadableMemberIsNotReportedAsAbsent(t *testing.T) {
	...
	// The regions member is optional -- --no-callable stores legitimately have
